...
$(document).ready(function(){
			...
			var ran=Math.floor(Math.random()*10000+1);
  			$("#firstOne").append('<li class="post" data-tag="'+arr[2][i]+'"><a href="product.html?title='+arr[1][i]+'&price='+random+'&sku='+ran+'&src='+arr[0][i]+'" target="_blank"><img src="' + arr[0][i] + '"></a><a href="product.html?title='+arr[1][i]+'&price='+random+'&sku='+ran+'&src='+arr[0][i]+'" target="_blank"><h4>'+arr[1][i]+'</h4></a><h5>'+'₹ '+random+'</h5><h6>SKU '+ran+'</h6></li>');
  			}
			$("h6").css({
				"float":"right",
				"color":"green"
